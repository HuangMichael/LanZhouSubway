...
public class WoMaterialCostController extends BaseController {
    ...
    public MyPage data( HttpServletRequest request, @RequestParam(value = "current", defaultValue = "0") int current, @RequestParam(value = "rowCount", defaultValue = "10") Long rowCount, @RequestParam(value = "searchPhrase", required = false) String searchPhrase) {
        ...
        return new PageUtils().searchBySortService(woMaterialCostSearchService, searchPhrase, SEARCH_PARAM_SIZE, current, rowCount, pageable);
    }
    ...
    public void exportExcel(HttpServletRequest request, HttpServletResponse response, @RequestParam("param") String param, @RequestParam("docName") String docName, @RequestParam("titles") String titles[], @RequestParam("colNames") String[] colNames) {
        List<WoMaterialCost> dataList = woMaterialCostSearchService.findByConditions(param, SEARCH_PARAM_SIZE);
